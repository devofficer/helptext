...
export const getMails = createAsyncThunk('feedbackApp/mails/getMails', async (routeParams, { getState }) => {
	routeParams = routeParams || getState().feedbackApp.mails.routeParams;
...
export const setFolderOnSelectedMails = createAsyncThunk(
	'feedbackApp/mails/setFolderOnSelectedMails',
	async (id, { dispatch, getState }) => {
		const { selectedMailIds } = getState().feedbackApp.mails;
...
export const toggleLabelOnSelectedMails = createAsyncThunk(
	'feedbackApp/mails/toggleLabelOnSelectedMails',
	async (id, { dispatch, getState }) => {
		const { selectedMailIds } = getState().feedbackApp.mails;
...
export const { selectAll: selectMails, selectById: selectMailById } = mailsAdapter.getSelectors(
	state => state.feedbackApp.mails
);

const mailsSlice = createSlice({
	name: 'feedbackApp/mails',
